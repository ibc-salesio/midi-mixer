...
public class mainclick_Activity extends Activity{
    ...
    public void onCreate(Bundle savedInstanceState) {//開いたら
       ...
       for (int i=0;i<16;i++){//とりあえず初期化
        ...
        setContentView(R.layout.auxclick_activity);
        nw.socket_make();
        ...
        on_off.setOnCheckedChangeListener(new OnCheckedChangeListener() {//変更がなされたとき
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked1) {
                if(isChecked1==true){
                    st.aon_c[st.ach_set]=1;
                    try {
						nw.send_packet("aCHon"+(st.ach_set+1)+"\n"+st.aon_c[st.ach_set]);
					} catch (IOException e) {
      ...
      //トグルスイッチここまで
        ...
        vol1.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar vol1, int progress, boolean fromUser) {
                ...
                //st.avol_db[st.ach_set]=tmp;//保存
                try {
					nw.send_packet("aCHdb"+(st.ach_set+1)+"\n"+String.valueOf(progress));
				} catch (IOException e) {
					// TODO 自動生成された catch ブロック
					e.printStackTrace();
				}
        ...
        chSpinner.setOnItemSelectedListener(new OnItemSelectedListener(){
            ...
            public void onItemSelected(AdapterView parent, View arg1,int arg2, long arg3) {
                ...
                //結果
                ch_v.setText("aCH"+(st.ach_set+1));//文字設定
                vol1.setProgress(st.avol_db[st.ach_set]);
                dBvol1.setText(String.valueOf(st.dblevel[st.avol_db[st.ach_set]])+"dB");
                if(st.aon_c[st.ach_set]==1){
        ...
        r.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                resetDialog.setMessage("AUXフェーダの設定がリセットされますがよろしいですか？");
                resetDialog.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        for (int i=0;i<16;i++){//とりあえず初期化
                            ...
                            st.avol_db[i]=0;
                            try {
								nw.send_packet("aCHon"+i+"\n"+st.aon_c[i]);
								nw.send_packet("aCHsolo"+i+"\n"+st.solo_c[i]);
								nw.send_packet("aCHdb"+i+"\n"+st.avol_db[i]);
								//Thread.sleep(0);
							} catch (IOException e) {
								// TODO 自動生成された catch ブロック
								e.printStackTrace();
        ...
        k.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                koushinDialog.setMessage("Mainフェーダの設定を更新しますか？");
